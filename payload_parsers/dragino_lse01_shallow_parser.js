  ...
  try {
        ...
        if(port==0x02){
          ...
          if((value & 0x8000)>>15 === 0)
            ...
            decode.temp_SOIL=((value-0x10000)/100).toFixed(2); // [CLEANFIX 2026-07-02] 16-bit two's-complement is -0x10000, not -0xFFFF (was +0.01°C off on negatives)
...
// ── [SOIL-DEPTH VARIANT — SHALLOW / Ρηχό] ─────────────────────────────────────────────
// Pins this single-sensor lse01 probe to soil depth slot 1, so two single-sensor
// probes can serve one field as Ρηχό (_1) + Βαθύ (_2). Only the emitted variable NAMES
// change; the decode above is identical to the base parser. Assign this parser to the
// device whose `type` tag is set to lse01_shallow (the create/update-field form does that).
(function () {
  var SOIL_DEPTH_RENAME = {
    soil_moisture: 'soil_moisture1',
    temp_soil: 'soil_temperature1',
    conduct_soil: 'conduct_soil1',
  };
  payload = payload.map(function (x) {
    return SOIL_DEPTH_RENAME[x.variable] ? Object.assign({}, x, { variable: SOIL_DEPTH_RENAME[x.variable] }) : x;
  });
})();
