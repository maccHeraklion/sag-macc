...
const CHECKS = [
  ...
  /* Το προοίμιο του crop_stage ΔΕΝ διαγράφηκε ποτέ. Στη v50.145 έμενε ανέγγιχτο· από τη
     v50.148 (T-STAGEPREAMBLE-01, απόφαση Μιχάλη) ΜΕΤΑΚΟΜΙΣΕ στην ημερήσια κάρτα μεθόδου.
     Και στις δύο περιπτώσεις το κείμενο ΠΡΕΠΕΙ να υπάρχει στον πυρήνα — αν λείψει, η
     τεκμηριωμένη διόρθωση της v50.116 («υδατικό στάδιο ≠ φαινολογία») έχει χαθεί.
     Η byte-προς-byte ταύτιση σταθεράς και σημείου εκπομπής ελέγχεται στο stagepreamble_check. */
  ["το προοίμιο του crop_stage ΥΠΑΡΧΕΙ ακόμη (v50.116· από v50.148 στην κάρτα μεθόδου)", f =>
...
const MUTATIONS = [
  ...
  // ΟΛΕΣ οι εμφανίσεις: από τη v50.148 το κείμενο υπάρχει σε ΔΥΟ σημεία (σταθερά + εκπομπή),
  // και μια αντικατάσταση μόνο της πρώτης άφηνε τον έλεγχο να περνά — δηλαδή δεν έλεγχε τίποτα.
  ["σβήστηκε το προοίμιο του crop_stage από ΟΛΑ τα σημεία", c => c.replaceAll(
